Rem
Rem $Header: sprepsql.sql 17-apr-2002.18:51:54 vbarrier Exp $
Rem
Rem sprepsql.sql
Rem
...
ttitle lef 'All Optimizer Plan(s) for this Hash Value' -
       skip 1 -
       lef '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' -
       skip 1 -
       lef 'Shows all known Optimizer Plans for this Hash value, and the Snap Id''s they' -
       skip 1 -
       lef 'were first found in the shared pool' -
       skip 1 -
       lef '-> ordered by Snap Id' -
       skip 2;

column plan_hash_value format 99999999999 heading 'Plan|Hash Value' just c;
column snap_id         format 9999999     heading 'Snap Id' just c;
...
select plan_hash_value
  ...
  from (select plan_hash_value
             , lag(plan_hash_value, 1, NULL) over (order by snap_id)  old_plan_hash_value
             , snap_id
             , cost
             , lag(cost, 1, NULL)            over (order by snap_id)  old_cost
             , optimizer
...
where old_cost            != cost
   or old_cost            is NULL
   or old_plan_hash_value != plan_hash_value
 order by snap_id;
