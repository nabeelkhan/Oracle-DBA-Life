...
#*******************************************************
su - applmgr -c "/usr/local/bin/scripts/forms_server_start.ksh $1" > /usr/local/bin/scripts/logs/forms_server_start_$1

#Check for errors
test=`grep -i error /usr/local/bin/scripts/logs/forms_server_start_$1|wc -l`
val=`expr $test`

if [ $val -gt 0 ]
then
   echo
   echo "ERROR: Errors found in /usr/local/bin/scripts/logs/forms_server_start_$1." 
   grep -i error /usr/local/bin/scripts/logs/forms_server_start_$1|mailx -s "$1 form shutdown error detected" dhurley@custom.com dburleson@custom.com
fi

#*******************************************************
# Stop the Concurrent manager
#*******************************************************
su - applmgr -c "/usr/local/bin/scripts/conc_mgr_start.ksh $1" > /usr/local/bin/scripts/logs/conc_mgr_start_$1

#Chek for errors
test=`grep -i error /usr/local/bin/scripts/logs/conc_mgr_start_$1|wc -l`
val=`expr $test`

if [ $val -gt 0 ]
then
   eho
   eho "ERROR: Errors found in /usr/local/bin/scripts/logs/conc_mgr_start_$1." 
   grep -i error /usr/local/bin/scripts/logs/conc_mgr_start_$1|mailx -s "$1 concurrent manager shutdown error detected" dhurley@custom.com dburleson@custom.com
fi

#*******************************************************
# Stop the Webserver
#*******************************************************
su - oracle -c "/usr/local/bin/scripts/webserver_start.ksh $1" > /usr/local/bin/scripts/logs/webserver_start_$1

#Chek for errors
test=`grep -i error /usr/local/bin/scripts/logs/webserver_start_$1|wc -l`
val=`expr $test`

if [ $val -gt 0 ]
then
   eho
   eho "ERROR: Errors found in /usr/local/bin/scripts/logs/webserver_start_$1." 
   grep -i error /usr/local/bin/scripts/logs/webserver_start_$1|mailx -s "$1 Webserver shutdown error detected" dhurley@custom.com dburleson@custom.com
fi

#*******************************************************
# Stop the listeners
#*******************************************************
su - oracle -c "/usr/local/bin/scripts/listener_start.ksh $1" > /usr/local/bin/scripts/logs/listener_start_$1

#Chek for errors
test=`grep -i error /usr/local/bin/scripts/logs/listener_start_$1|wc -l`
val=`expr $test`

if [ $val -gt 0 ]
then
   eho
   eho "ERROR: Errors found in /usr/local/bin/scripts/logs/listener_start_$1." 
   grep -i error /usr/local/bin/scripts/logs/listener_start_$1|mailx -s "$1 Webserver shutdown error detected" dhurley@custom.com dburleson@custom.com
fi
