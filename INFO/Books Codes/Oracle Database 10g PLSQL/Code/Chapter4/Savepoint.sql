...
PROMPT
PROMPT
PROMPT ** Run an anonymous block with two savepoints, and a rollback to the second
PROMPT
...
BEGIN
   ...
   INSERT INTO inventory (isbn, status, status_date, amount)
   VALUES ('12345678', 'BACKORDERED', null, 1100);
...
PROMPT
PROMPT
PROMPT ** Query the tables in the anonymous block to confirm the rollback worked
PROMPT

SELECT b.title, i.status
FROM books b, inventory I
WHERE b.isbn = '12345678'
AND b.isbn = i.isbn;
