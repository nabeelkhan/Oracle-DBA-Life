...
PROMPT
PROMPT
PROMPT ** Select the rowid, first_name, and last name before the update.
PROMPT **  The records selected are the authors for this book.
PROMPT

      COL first_name FORMAT A7
      COL last_name FORMAT A10

      SELECT a.rowid, a.first_name, a.last_name
      FROM authors a, books b
      WHERE b.isbn = '72230665'
      AND (
         a.id = b.author1
         OR
         a.id = b.author2
...
PROMPT
PROMPT ** Update the Authors table, converting author names to uppercase
PROMPT

DECLARE

/* Retrieve the rowid, first name, and last name of the authors
   of this book into the cursor */

   CURSOR author_rowid_cur
   IS
      SELECT a.rowid
      FROM authors a, books b
      WHERE b.isbn = '72230665'
...
BEGIN
   ...
   /* Loop throuth the records retrieved by the cursor and convert
      the first and last names to uppercase */

   FOR y IN author_rowid_cur
   LOOP
...
PROMPT
PROMPT
PROMPT ** Select the rowid, first_name, and last name after the update.
PROMPT **  The records selected are the authors for this book.
PROMPT
      COL first_name FORMAT A7
      COL last_name FORMAT A10
