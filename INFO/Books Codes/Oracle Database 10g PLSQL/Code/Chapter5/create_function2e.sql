 ...
 *
 * This script demonstrates using an object type can
 * not be assigned a %ROWTYPE record type.
 */
...
CREATE OR REPLACE TYPE individual_record_type AS OBJECT
...
CREATE OR REPLACE TYPE individual_record AS OBJECT
  ...
  (individual     INDIVIDUAL_RECORD_TYPE)
...
CREATE OR REPLACE TYPE BODY individual_record AS
  ...
  BEGIN
    self.individual_id := individual.individual_id;
    self.first_name := individual.first_name;
    self.middle_initial := individual.middle_initial;
...
DECLARE
  ...
  individual_type INDIVIDUAL_RECORD_TYPE;
  ...
  BEGIN
    ...
    FOR i IN c(individual_id_in) LOOP
      ...
      RETURN   individual_record(individual_type);
    END LOOP;
