...
PROMPT
PROMPT Turn tracing on, and run two identical selects
PROMPT with different literals, then turn tracing
PROMPT off.
PROMPT
...
PROMPT
PROMPT Turn tracing on, run the same two queries using
...
PROMPT
PROMPT Create a procedure to test variables being treated
PROMPT as bind variables
PROMPT

CREATE OR REPLACE PROCEDURE bind_test (
   i_author_first_name IN AUTHORS.FIRST_NAME%TYPE)
IS
   v_author_last_name AUTHORS.LAST_NAME%TYPE;
BEGIN
   SELECT last_name
   INTO v_author_last_name
   FROM authors
   WHERE first_name = i_author_first_name;

   DBMS_OUTPUT.PUT_LINE(i_author_first_name
                        ||' has a last name of '
                        ||v_author_last_name);
EXCEPTION
   WHEN OTHERS
...
PROMPT
PROMPT Turn tracing on, execute the bind_test procedure,
PROMPT then turn tracing off
PROMPT
