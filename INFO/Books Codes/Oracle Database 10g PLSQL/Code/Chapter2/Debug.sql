...
exec clean_schema.triggers
exec clean_schema.procedures
...
PROMPT
PROMPT *********** THIS WILL NOT COMPILE *************
PROMPT

CREATE OR REPLACE PROCEDURE author_books_sel
IS
...
BEGIN
   ...
   OPEN auth_cur;
   LOOP
      FETCH auth_cur INTO first_name, v_last_name, v_row_count;
...
PROMPT
PROMPT ** Create an BEFORE UPDATE trigger on the AUTHORS table
PROMPT
...
PROMPT
PROMPT ** Create a procedure that will cause the author_trig to fire
PROMPT

CREATE OR REPLACE PROCEDURE author_first_name_upd (
   i_author_id IN AUTHORS.ID%TYPE,
   i_first_name IN AUTHORS.FIRST_NAME%TYPE)
IS
BEGIN

   UPDATE authors a
   SET a.first_name = UPPER(i_first_name)
   WHERE a.id = i_author_id;
