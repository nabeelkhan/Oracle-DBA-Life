...
CREATE OR REPLACE PROCEDURE get_scheduler_details(
...
BEGIN
   OPEN cv_job_details FOR
      ...
      WHERE job_name = UPPER(i_job_name);
EXCEPTION
   WHEN OTHERS
   THEN
      DBMS_OUTPUT.PUT_LINE(SQLERRM);
...
BEGIN
   ...
   DBMS_SCHEDULER.CREATE_JOB (JOB_NAME => 'EXAMPLE1',
                  JOB_TYPE => 'STORED_PROCEDURE',
                  JOB_ACTION => 'EMAIL_MANAGER.INLINE_EMAIL',
                  START_DATE => TRUNC(SYSTIMESTAMP + 1),
                  REPEAT_INTERVAL => 'FREQ=MINUTELY;INTERVAL=30',
                  END_DATE => SYSTIMESTAMP+300,
                  ENABLED => TRUE,
                  AUTO_DROP => TRUE,
                  COMMENTS => 'Send E-Mail on 30 min intervals');
...
EXEC GET_SCHEDULER_DETAILS('EXAMPLE1', :v_job_details)
...
BEGIN
   ...
   DBMS_SCHEDULER.CREATE_JOB (JOB_NAME => 'EXAMPLE2',
                  JOB_TYPE => 'PLSQL_BLOCK',
                  JOB_ACTION => v_statement,
                  START_DATE => SYSTIMESTAMP,
                  REPEAT_INTERVAL => 'FREQ=WEEKLY;BYDAY=SUN; BYHOUR=23; BYMINUTE=45',
                  END_DATE => SYSTIMESTAMP+300,
                  ENABLED => TRUE,
                  AUTO_DROP => TRUE,
                  COMMENTS => 'Delete e-mails once per week at 23:45 Sunday');
   COMMIT;
END;
/

VARIABLE v_job_details REFCURSOR
EXEC GET_SCHEDULER_DETAILS('EXAMPLE2', :v_job_details)
